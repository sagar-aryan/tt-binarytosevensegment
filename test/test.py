...
async def uart_send_byte(dut, data: int):
    ...
    # START bit
    dut.ui_in.value = dut.ui_in.value & ~1  # ui_in[0] = 0
...
async def send_packet(dut, b0, b1, b2, b3):
    ...
    # Wait for ACK start-bit (TX goes low) — timeout after 50000 clock edges
    for _ in range(50000):
        await RisingEdge(dut.clk)
        if int(dut.uo_out.value) & 1 == 0:   # uo_out[0] = TX
            dut._log.info("ACK start bit detected on TX")
            break

    # Let the full ACK byte (11 baud periods) transmit
    await Timer(BAUD_PERIOD_NS * 11, units="ns")
...
async def test_project(dut):
    ...
    # Test 2: send decimal 123456  (0x00_01_E2_40)
    ...
    # Test 3: send decimal 123456789  (0x07_5B_CD_15)
